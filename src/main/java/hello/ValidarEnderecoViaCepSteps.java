...
public class ValidarEnderecoViaCepSteps {
    ...
    @Então("deverá mostrar o nome da uf {string}")
    public void deveraMostrarAUf(String estado){
        String estadoEsperado = response.then().extract().jsonPath().get("uf");
        Assert.assertEquals(estado, estadoEsperado);
    }
